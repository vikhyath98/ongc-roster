...
delete from manifest_request_items
  where employee_id in (select id from employees where emp_id like 'TEST-S2-%')
     or replacing_employee_id in (select id from employees where emp_id like 'TEST-S2-%')
     or manifest_request_id in (select id from manifest_requests where notes like 'TEST-S2-%');
-- 8. manifest_requests
